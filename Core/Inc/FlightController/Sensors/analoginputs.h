...
public:
    ...
    static constexpr uint32_t ChannelCount = 1; //2
    static constexpr uint32_t SamplesPerChannel = 32;
    static constexpr uint32_t DmaBufferSize = ChannelCount * SamplesPerChannel;
...
public:
    ...
    uint32_t GetRaw(Channel channel) const;
    uint32_t GetFilteredRaw(Channel channel) const;
...
private:
    ...
    uint32_t m_dmaBuffer[DmaBufferSize]{};

    uint32_t m_raw[ChannelCount]{};
